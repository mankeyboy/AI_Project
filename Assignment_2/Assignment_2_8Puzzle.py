...
def idaSolver(startState, choice):
    bound = h(startState,choice)
    p = (-25)
    nodes = []
    nodes.append(0)
    while True :
        ret = search(startState, 0, bound,choice, nodes, None)
        if ret == p :
            return (bound,nodes)
        elif ret == float('inf') :
            return (-100)
...
def h(State,choice):
    ...
    if choice == 'a' :
        ...
        for i in range(len(State)) : #B Heuristic 
...
def search(state, g, bound, choice,nodes,parent):
    for i in range(len(state)):
        if state[i] == 0 :
            ePos = i
            break 
    ...
    for i in actions :
        nodes[0] += 1
        newS = newState(state,i,ePos,choice)
        # print newS[0], bound
        if(newS[0] != parent):
            ret = search(newS[0],g+1,bound,choice,nodes,state)
            r = -25
            if ret == r :
                return ret
            if ret < mini :
...
if __name__ == '__main__':
    print "Input the Solver type : A*(1)/ID A*(2)"
    choice = raw_input()
    if (len(choice) > 1 or choice > '2' or choice < '1'):
        print "Wrong Input/ No Such solver"
    elif choice == '1':
        ...
        if(len(choice) > 1 or choice > 'c'):
            ...
            for i in range(len(initialState)) : 
                solN = idaSolver((initialState[i]),choice)
                print "Initial State, f(n)"
                print initialState[i] , solN[0], solN[1][0]        
